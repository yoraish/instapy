...
def init_timelapse(slow_factor = 2):
    ...
    fps = 30 # fixed
    defWait = 1/fps # fixed
    slowFactor = slow_factor # changeable
    ...
    * export video to somewhere
    ...
    while True:
        ...
        if state == 3:
            ...
            if image_couter < numImages and datetime.datetime.now() >= last_capture_time + wait_time_delta:

                camera.capture('/home/pi/Desktop/instapy/photos/' +str(image_couter)+ '.jpg')
                print('pic num', str(index))
                last_capture_time = datetime.datetime.now()

            if datetime.datetime.now() >= last_update_time + datetime.timedelta(minutes = 1):

                post_server("CAPTURING, DONE SOON")
                last_update_time = datetime.datetime.now()
            
            if numImages == image_couter:
